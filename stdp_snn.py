...
def f_pre(x, w_min, alpha=0.0):
    return (x - w_min) ** alpha


def f_post(x, w_max, alpha=0.0):
    return (w_max - x) ** alpha


# 参数、SNN网络结构、
w_min, w_max = -1.0, 1.0
tau_pre, tau_post = 2.0, 2.0
N_in, N_out = 4, 3
T = 128
batch_size = 2
lr = 0.01
net = nn.Sequential(layer.Linear(N_in, N_out, bias=False), neuron.IFNode())
...
plt.subplot(6, 1, 5)
plt.plot(t, trace_pre, c=cmap(4))
plt.plot(t, trace_post, c=cmap(5))
plt.xlim(-0.5, T + 0.5)
plt.ylabel("$tr_{pre}$ and $tr_{post}$", rotation=0)
plt.yticks([trace_pre.min().item(), trace_pre.max().item()])
plt.xticks([])
